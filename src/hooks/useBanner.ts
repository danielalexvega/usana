...
import { LanguageCodenames, type CTA } from "../model";
...
export interface UseBannerResult {
  banner: Replace<CTA, { elements: Partial<CTA["elements"]> }> | null;
  isLoading: boolean;
  error: string | null;
}
...
export const useBanner = (isPreview: boolean, lang: string | null, slug: string | null): UseBannerResult => {
  ...
  const [banner, setBanner] = useState<Replace<CTA, { elements: Partial<CTA["elements"]> }> | null>(null);
  ...
  const handleLiveUpdate = useCallback((data: IUpdateMessageData) => {
    if (banner) {
      applyUpdateOnItemAndLoadLinkedItems(
        ...
        if (updatedItem) {
          setBanner(updatedItem as Replace<CTA, { elements: Partial<CTA["elements"]> }>);
        }
      }).catch((err) => {
  ...
  useEffect(() => {
    ...
    createClient(environmentId, apiKey, isPreview)
      ...
      .toPromise()
      .then(res => {
        const item = res.data.item as Replace<CTA, { elements: Partial<CTA["elements"]> }> | undefined;
        if (item) {
          setBanner(item);
          setError(null);
        } else {
